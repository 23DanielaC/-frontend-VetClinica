<template>
  <div class="relative z-10 space-y-6">
        ...
        <table class="w-full min-w-[980px]">
          <thead>
            <tr class="bg-slate-50/80 border-b border-slate-100">
              <th v-for="col in columns" :key="col" class="pag-th">{{ col }}</th>
            </tr>
          </thead>
            ...
            >
              ...
              <td class="pag-td">
                <div class="flex items-center justify-end gap-0.5">
                  ...
                  <button type="button" title="Editar" class="pag-action pag-action--green" @click="openModal(pago)">
      ...
      <div class="md:hidden p-4 space-y-3">
        ...
        >
          ...
          <div class="flex gap-1 mt-3 pt-3 border-t border-slate-100">
            ...
            <button type="button" class="pag-action pag-action--green flex-1" @click="openModal(pago)"><PencilSquareIcon class="w-4 h-4 mx-auto" /></button>
    ...
    <!-- Modal CRUD -->
    <CrudModal v-model="showModal" :title="editing ? 'Editar Pago' : 'Registrar Pago'" @confirm="save">
      <div class="space-y-4">
        <div>
...
function onFacturaChange() {
  ...
  if (f && !editing.value) form.value.monto = f.total
}
...
function save() {
  ...
  if (editing.value) {
    store.updateItem(store.pagos, editing.value.id, payload)
  } else {
